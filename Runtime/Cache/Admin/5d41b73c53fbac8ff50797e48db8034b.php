...
<div class="panel-main" style="float:left;">
    ...
    <div class="with-padding">
        ...
        <button class="btn ajax-post" target-form="ids" url="<?php echo U('setstatus',array('status'=>1,'Model'=>'action'));?>" >启 用</button>
        <button class="btn ajax-post" target-form="ids" url="<?php echo U('setstatus',array('status'=>0,'Model'=>'action'));?>">禁 用</button>
        <button class="btn ajax-post confirm" target-form="ids" url="<?php echo U('setStatus',array('status'=>-1,'Model'=>'action'));?>">删 除</button>
    </div>
	<!-- 数据列表 -->
...
<table class="table table-bordered table-striped tabel-center">
    ...
    <tbody>
		<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><a href="<?php echo U('ActionLimit/limitlist',array('action'=>$vo['name']));?>" title="查看限制"><?php echo ($vo["name"]); ?></a></td>
