...
class MemberController extends Controller
{
    ...
    {
        //获取参数
        $aUsername = $aNickname = $username = I('post.username', '', 'op_t');
        $aPassword = time();
        ...
        //$this->ajaxReturn(array("data"=>I("post."),"a"=>$aUsername));
        ...
        if (IS_POST) { //注册用户
            ...
            if (check_verify_open('reg')) {
				if(C('_USERCONFIG_EMAIL_VERIFY_TYPE')!=2  || C('_USERCONFIG_MOBILE_VERIFY_TYPE')!=1){
            ...
            if (($aRegType == 'mobile' && modC('MOBILE_VERIFY_TYPE', 0, 'USERCONFIG') == 1) || (modC('EMAIL_VERIFY_TYPE', 0, 'USERCONFIG') == 2 && $aRegType == 'email')) {
                $uid = is_login()?is_login():0;
               // $this->ajaxReturn(array("zhi"=>$aUsername.'--'.$aRegType.'--'.$aRegVerify.'--'.$uid));
                if (!D('Verify')->checkVerify($aUsername, $aRegType, $aRegVerify, $uid)) {
                    $str = $aRegType == 'mobile' ? '手机' : '邮箱';
                    $this->error($str. '验证失败');
                }
            }
            $lists = M("UcenterMember")->where(array("mobile"=>$aUsername))->find(); //通过账号密码取到uid
            if($lists)
            {
                if($lists['id'])
                {
                    $data=D('Member')->login($lists['id'], false, $aRole,1); //登陆
                }
                else {
            ...
            if (0 < $uid) { //注册成功
                ...
                $this->success('注册成功', U('App/demo/index'));
            } else { //注册失败，显示错误信息
    ...
    public function login()
    {
        ...
        if (IS_POST) {
            ...
            $aVerify = I('post.reg_verify', '', 'op_t');
            $aRole = 1 ;

            $list = M("Verify")->where(array("account"=>$aUsername))->find();
            if($list['verify']!=$aVerify)
            {
                $this->error('验证码输入错误');
            }
            ...
            */
            $uid = M("UcenterMember")->where(array("mobile"=>$aUsername))->find(); //通过账号密码取到uid
            //print_r($uid);exit;
            if ($uid['id'] ) {
                D('Member')->login($uid['id'], false, 1); //登陆
                $this->success('登录成功', U('App/demo/index'));
            } else {
                $this->error($result['info']);
            }
        } else { //显示登录页面
    ...
    public function mi($username = '', $email = '', $verify = '')
    {
        ...
        if (IS_POST) { //登录验证
            ...
            $this->success('密码找回邮件发送成功,请进入您的邮箱点击验证链接');
        } else {
            if (is_login()) {
    ...
    public function doReset($password, $repassword,$verify='')
    {
        ...
        switch ($type) {
            case 'mobile':
              $content = modC('SMS_CONTENT', '{$verify}', 'USERCONFIG');
                $content = str_replace('#code#', $verify, $content);
                $content = str_replace('#company#', modC('SMS_COMPANY', '', 'USERCONFIG'), $content);
				$content = str_replace('#app#', modC('SMS_APP', '', 'USERCONFIG'), $content);
				$content = str_replace('#tel#', modC('SMS_TEL', '', 'USERCONFIG'), $content);
				$content = str_replace('#hour#', modC('SMS_HOUR', '', 'USERCONFIG'), $content);
				$content = str_replace('#name#', $account, $content);
            ...
            case 'email':
                ...
                $content = str_replace('#code#', $verify, $content);
                $content = str_replace('#title#', $account, $content);
