...
class AdminController extends Controller
{
     ...
     * @author 朱亚杰  <zhuyajie@topthink.net>
     */
    protected function true_delete_user($model, $where = array(), $uid = array())
    {
		
		if(M($model)->where($where)->delete()){
			M('UcenterMember')->where($uid)->delete();
			M('UserRole')->where($where)->delete();
			M('Avatar')->where($where)->delete();
			M('RankUser')->where($where)->delete();
			M('Field')->where($where)->delete();
            $this->success('删除成功');
		}else{
		    $this->error('删除失败');
		}
                ...
                foreach ($menus['main'] as $key => $item) {
                    ...
                    if ($item['title'] == $nav_first_title) {
                        ...
                        foreach ($groups as $g) {
                            ...
                            $menuList = M('Menu')->where($map)->field('id,pid,title,url,tip,icon')->order('sort asc')->select();
