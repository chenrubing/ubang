...
class ArticleController extends AdminController {
    ...
    public function index($cate_id = null){
		...
		$ifpage         =   get_category($cate_id, 'ifpage');
		...
		if(!$ifpage){
			...
			$info =  D('Category')->where("id = '$cate_id'")->find(); 	
			$template = "page";
		...
		if(!C('CATEGORY_JSON')){
			$Category = D('Category');
   			$clist  = $Category->order('`pid` asc, `sort` asc, id asc')->select(); 	
      	 	$tree = $Category->Tree($clist);
