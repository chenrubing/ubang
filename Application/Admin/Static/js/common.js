...
$(document).on('click','.ajax-get', function() {
		...
		if($(this).hasClass('confirm')&&$(this).hasClass('btn')){
			//按钮警告文字 郑薏玮
			$(this).addClass('btn-danger').text('确认要执行该操作吗').removeClass('confirm');
			return false;
    ...
    //ajax post submit请求
   	$(document).on('click','.ajax-post', function() {
		...
		if($(this).hasClass('confirm')&&$(this).hasClass('btn')){
			//按钮警告文字 郑薏玮
			$(this).addClass('btn-danger').text(confirm_info).removeClass('confirm');
			return false;
		}else if($(this).hasClass('confirm')){
        ...
        if (($(this).attr('type') == 'submit') || (target = $(this).attr('href')) || (target = $(this).attr('url'))) {
			...
			if(!target_form){
				//如果不设置 target-form 也可以获取到上一级的form的class //郑薏玮
		  	   form =  $('.' + $(this).closest('form').attr('class')) ;
			}
