...
class AdminListBuilder extends AdminBuilder
{
    ...
    */
	...
	public function planStatus($name = 'shopstatus', $title = '订单状态',$map)
    { 
        return $this->key($name, $title, 'planStatus', $map);
    }
...

    {
        ...
        $setStatusUrl = $this->_setStatusUrl;
        $that = & $this;
        $this->convertKey('planStatus', 'html', function ($value, $key, $item) use ($setStatusUrl, $that) {
            ...
            return "<a href=\"javascript:void(0);\" data-remote=\"{$url}\" title='订单状态管理器'  data-toggle=\"modal\">$text</a>";
        });
        ...
        $this->convertKey('image', 'html', function ($value, $key, $item) {
            ...
            if(intval($value)){//value是图片id
                ...
                $html="<div class='popup-gallery'><a title=\"查看大图\" href=\"$sc_src\"><img src=\"$sc_src\"/ style=\"width:50px;height:50px\"></a></div>";
