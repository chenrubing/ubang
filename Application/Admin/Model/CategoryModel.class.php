...
class CategoryModel extends Model{
 ...
 /**
    ...
    public function update(){
		...
		//获取上级分类
		if($data[pid]){
	   		$upDB = $this -> where("id = $data[pid]")->find();
		}
        /* 添加或更新数据 */
        if(empty($data['id'])){
            $res = $this->add($data);
			if(!$data['pid']){
				$this -> where("id = $res")->setField('path',"$res");
			}else{
				$this -> where("id = $res")->setField('path',"$upDB[path],$res");
			}
        }else{
            $res = $this->save($data);
			if(!$data['pid']){
				$this -> where("id = $data[id]")->setField('path',"$data[id]");
			}else{
				$this -> where("id = $data[id]")->setField('path',"$upDB[path],$data[id]");
			}
        }
