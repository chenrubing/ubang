...
class ArticleController extends CmsController
{
    ...
    public function index()
    {
        ...
        $dcument = D('Document');
        $dlis = $dcument->where($map)->order('create_time')->limit(4)->select();
        $map['status']=1;
    ...
    public function article($id=null)
    {
        ...
        $map['do.status']=1;
        $map['do.id']=$id;
        $list=$dcument->table("ocenter_document do,ocenter_document_article ae")->where("do.id=ae.id")->where($map)->find();
        //dump($list);
    ...
    private function category($id = 0)
    {
        ...
        $categoryModel = new \Cms\Model\CategoryModel();
        /* 获取分类信息 */
        $category = D('Cms/Category')->info($id);
