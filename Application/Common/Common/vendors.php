...
function sendSMS($mobile, $text, $time = '', $mid = '')
{
	...
	$apikey = modC('SMS_APIKEY', '', 'USERCONFIG');
	
	$str = str_replace('>',"&gt;",$str);
	

    if (empty($http) || empty($text)) {
    ...
    (
		'apikey' => $apikey,
        'mobile' => $mobile, //号码
        'text' => $text, //内容 如果对方是utf-8编码，则需转码iconv('gbk','utf-8',$content); 如果是gbk则无需转码
        'encode' => 'utf8',
    );
...
function postSMS($url, $data = '')
{
  	 while(list($k, $v) = each($data)) {
		...
		$head.="Content-Length: ".strlen(trim($post))."\r\n";
		$head.="\r\n";
		$head.=trim($post);
		$write=fputs($fp,$head);
		$header = "";
		...
		$msg = htmlspecialchars($str[msg], ENT_NOQUOTES)	;
