...
function is_login()
{
    ...
    if (empty($user)) {
		...
		session('form_uid',null);//登陆即情况推荐人uid
...
function get_file($file_id, $field = null)
{
    ...
    if (is_bool(strpos($file['path'], 'http://'))) {
        $file['path'] = fixAttachUrl($file['path']);
    }
...
//获取微店配置
function shopCofing($field){
	if($field){
		$config =  M('ShopConfig')->where(array('id'=>1))->getField($field);
	}else{
...
//获取指定用户的推荐人或者推荐人数组
function get_Usertree($uid,$tree=''){
	
	if($tree){
		$user_tree =  M('Member')->where(array('uid'=>$uid))->getField('user_tree');
		$ShopPostup =  M('ShopConfig')->where(array('id'=>1))->getField('postup');//获取佣金比例配置
		$postup = explode("\r\n",$ShopPostup);//将比例组成数组
		$backtree = explode('-',$user_tree);
		$backtree = array_slice($backtree,-$tree,$tree);
		rsort($backtree);
		foreach($backtree as $k =>$v){
			$data[$k][uid] = $v;
			$data[$k][scale] = $postup[$k];
		}

		return $data;
	}else{
		$form_uid =  M('Member')->where(array('uid'=>$uid))->getField('form_uid');
		return $form_uid;
	}
...
//查询订单状态
function getStatus($uid,$service_id,$type='form_c2b_id'){
	$text = array("-3"=>"商家取消订单","-2"=>"用户取消订单","0"=>"暂无回馈","1"=>"商家已回馈","2"=>"交流中","3"=>"服务中","4"=>"订单完成","-1"=>"删除订单");
	$order = M('ServiceOrder')->where(array('uid'=>$uid,$type=>$service_id))->getField('status');
	
	if(!$order){
		$order = M('ServicePlan')->where(array('uid'=>$uid,$type=>$service_id))->getField('status');
		$order || $order = 0;
	}
	return $text[$order];
}
