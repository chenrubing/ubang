...
class MemberModel extends Model
{
    ...
    */
    ...
    /**
     * 登录指定用户
     * @param  integer $uid 用户I
     * @param bool $remember
     * @param int $role_id 有值代表强制登录这个角色
     * @return boolean      ture-登录成功，false-登录失败
     */

    public function login($uid, $remember = false,$role_id=0,$denlu=0)
    {
        ...
        $infos= M("Member")->where(array("uid"=>$uid))->field("uid,nickname,img")->find();
        if (!empty($step) && $step != 'finish') {
            header('Content-Type:application/json; charset=utf-8');
            $data['status'] = 1;
            $data['list'] = $infos;
            $data['info'] = $denlu ? "登录成功！":"注册成功！";
